...
skf() {
  ...
  if [ $mode = "n" ]; then
    ...
    git log --oneline  --color=always | fzf --ansi --multi --preview 'git show {+1} --color=always' --preview-window=up +s
 else
...
gli() {
  ...
  local fzf=(
    fzf
    --ansi --no-sort --no-height --reverse --tiebreak=index
    --preview "f() { set -- \$(echo -- \$@ | grep -o '[a-f0-9]\{7\}'); [ \$# -eq 0 ] || git show --color=always \$1 $filter; }; f {}"
    --bind "j:down,k:up,ctrl-d:preview-down,ctrl-p:preview-up,ctrl-f:preview-page-down,ctrl-b:preview-page-up,ctrl-q:abort,ctrl-m:execute:
